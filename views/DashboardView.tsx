...
import { MOCK_GOALS, MOCK_CLIENTS, MOCK_MATERIALS } from '../constants';
import { 
...
const DashboardView: React.FC<DashboardViewProps> = ({ onNavigate }) => {
  ...
  // Simulação de Dados Dinâmicos baseados no intervalo selecionado
  // Em um app real, aqui faríamos um fetch filtrando por essas datas
  const dashboardData = useMemo(() => {
    // Cálculo mockado para demonstrar reatividade
    const diffTime = Math.abs(new Date(endDate).getTime() - new Date(startDate).getTime());
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) || 1;
    
    // Simula variação proporcional ao tempo selecionado
    const baseRevenue = 250 * diffDays; 
    const revenue = Math.min(baseRevenue, 12000); // Teto para o mock
    const profit = Math.round(revenue * 0.72);
    const costs = revenue - profit;
    const ticket = 350;
    const volume = Math.round(revenue / ticket);
    const variation = "+12%";
  ...
  const metaFaturamento = MOCK_GOALS.find(g => g.type === 'faturamento') as Goal;
  const progress = (dashboardData.revenue / metaFaturamento.target) * 100;
  const restante = metaFaturamento.target - dashboardData.revenue;
  
  const riskCount = MOCK_CLIENTS.filter(c => c.status === 'EM RISCO').length;
